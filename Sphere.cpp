...
rt::Sphere::rayIntersection( const Ray& ray, Point3& p )
{
  ...
  double a = 1;
  double b = 2 * (pa.dot(ray.direction));
  double c = (pa.dot(pa)) - pow(radius,2);
  float delta = (b * b) - 4 * a * c;
  if(delta < 0) 
    return 1.0f;
  float t1 = (-2 * pa.dot(ray.direction) - sqrt(delta)) / 2;
  float t2 = (-2 * pa.dot(ray.direction) - sqrt(delta)) / 2;
  if(t2 < 0) 
    return 1.0f;
  if(t1 < 0)
